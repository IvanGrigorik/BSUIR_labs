...
impl Default for Application {
    fn default() -> Self {
        let source_image = load_image_from_path(path::Path::new("image/vilki.jpg")).unwrap();
...
fn find_bar_chart(image: &ColorImage) -> [usize; (u8::MAX as usize) + 1] {
    ...
    find_threshold(bar_chart);

    bar_chart
}

fn find_threshold(bar_chart: [usize; (u8::MAX as usize) + 1]){ //-> u8 {

    //let maximum_first_half: u8 = *bar_chart[0..(bar_chart.len()/2 as usize)].iter().max().unwrap() as u8;
    //let maximum_second_half: u8 = *bar_chart[bar_chart.len()/2 as usize..bar_chart.len()].iter().max().unwrap() as u8;
    ...
    println!("{}", (first_half_index_max+second_half_index_max)/2);

    //(first_half_index_max+second_half_index_max)/2 as 
}

fn compute_binarized_image(
    image: &ColorImage,
    ...
    for y in 0..image.height() {
        for x in 0..image.width() {
            ...
            if target_gray_level < threshold {result.pixels[i] = Color32::from_gray(0);}
            else {result.pixels[i] = Color32::from_gray(255);}
...
fn generate_low_passed_image(image: &ColorImage) -> ColorImage {
    let mut result = compute_low_pass_filter(image);
    
    for _ in 0..1 {
        result = compute_low_pass_filter(&result);
    }
                            ...
                            if self.target_brightness.min >= self.target_brightness.max {
                                ...
                                let image = match self.state {
                                    ...
                                    ViewState::ContrastImage =>
                                        compute_binarized_image(//compute_contrast_image(
                                            &generate_low_passed_image(&self.gray_image),
